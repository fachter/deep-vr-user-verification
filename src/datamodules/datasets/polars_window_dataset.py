...
class WindowDataset:
    ...
    def klasses(self) -> np.ndarray:
        if not hasattr(self, "_klasses"):
            self._klasses = np.sort(self.ground_truth.unique())
    ...
    def _scale_data(self, X):
        ...
        for index, col in enumerate(X.columns):
            data[col] = (X[col] - self.means[index]) / self.stds[index]
        return data
    ...
    def _load_and_set_data(self):
        ...
        def load_data_file(file_path):
            ...
            df = pl.scan_ipc(file_path).rename(columns=lambda col: col.replace("delta_", ""))
